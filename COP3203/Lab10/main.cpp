...
void ReadFile(const char *filename, vector<Color> &outVector)
{
  ...
  while (getline(file, line))
  {
    if (line.empty())
      break;
    std::size_t index_split = line.find(" ");

    string name(line.begin(), line.begin() + index_split);
    string value(line.begin() + index_split + 1, line.end());

    Color c;
    c.SetName(name.c_str());
    c.SetValue(stoi(value)); // This should also convert the values, internally
